        ...
        try:
            ...
            resp = session.post(
                ...
                json={
                    "model": self.model,
                    "messages": [
                        {"role": "system", "content": "你是专业金融新闻分析师，输出结构化分析。"},
            ...
            if resp.status_code == 200:
                ...
                if "choices" in result and len(result["choices"]) > 0:
                    logger.info(f"DeepSeek分析成功: {news_title}")
...
_deepseek_analyzer: Optional[DeepSeekAnalyzer] = None
...
def init_deepseek_analyzer(
    ...
    global _deepseek_analyzer
    _deepseek_analyzer = DeepSeekAnalyzer(
        api_key=api_key,
        model=model,
        feishu_webhook_url=feishu_webhook_url,
        keyword=keyword
    )
    logger.info(f"DeepSeek大模型分析器已初始化，模型: '{model}'")


def get_deepseek_analyzer() -> Optional[DeepSeekAnalyzer]:
    return _deepseek_analyzer
